    ...
    <body style="background-color: white;">
        <div class="container">
            ...
            <?php
                ...
                $action=$_GET["action"];
                $famille = array(); 
                $famille = one_select_famille_de_risque($action); 
            ?>
            <form action="controleur_modif.php" method="post">
                <input type="hidden" name="mode" value="6">    

                <label for="Id_Famille_de_risque">Num</label>
                <input type='text' name="Id_Famille_de_risque" id="Id_Famille_de_risque" value="<?=$famille['Id_Famille_de_risque']?>"  readonly="readonly">
                <label for="famille">Famille</label>
                <input type="text" name="famille" id="famille" value="<?=$famille['famille']?>">	
